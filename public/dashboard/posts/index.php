<?php 
  ...
  $condition = "inner join users on posts.last_update_by = users.id left join tags_posts on posts.id = tags_posts.post_id left join tags on tags.id = tags_posts.tag_id";

  if(isset($_GET['delete_post']) && !empty($_GET['delete_post'])){
    $posts->delete($_GET['delete_post']);
  }
...
<body class="app sidebar-mini pace-done"> 
    ...
    <div class="app-title">
        <div>
          <h1><i class="fa fa-list-alt"></i> All Posts</h1>
        </div>  
